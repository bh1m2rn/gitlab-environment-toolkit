...
- `*_data_disks` - The main setting for each node group.
  ...
  - `stop_instance_before_detaching` - Set this to true to ensure that the target instance is stopped before trying to detach the volume. Stops the instance, if it is not already stopped. **Optional**, defaults to false
