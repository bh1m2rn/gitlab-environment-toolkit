...
- `*_data_disks` - The main setting for each node group.
  ...
  - `volume_attachment_skip_destroy` - Set this to true if you do not wish to detach the volume from the instance to which it is attached at destroy time, and instead just remove the attachment from Terraform state. This is useful when destroying an instance which has volumes created by some other means attached. **Optional**, defaults to false
  - `volume_attachment_stop_instance_before_detaching` - Set this to true to ensure that the target instance is stopped before trying to detach the volume. Stops the instance, if it is not already stopped. **Optional**, defaults to true
